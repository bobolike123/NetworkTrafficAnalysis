...
def load_data(root_path):
    ...
    input_texts = []
    label_packets = []
    file_list = glob.glob(root_path + '/CICIDS*.pkl')
...
def dataset_generator(packets, labels, indices, batch_size):
    ...
    Ybatch = np.zeros((batch_size, CLASSNUM), dtype=np.int64)
    batch_idx = 0
    while True:
        for idx in indices:
            ...
            Ybatch[batch_idx] = np_utils.to_categorical(labels[idx], num_classes=CLASSNUM)
            batch_idx += 1
...
def get_indices(target_texts):
    ...
    attack_indices = [np.where(target_texts == i)[0] for i in range(1, CLASSNUM)]
    ...
    valid_attack_indices = np.concatenate(
        [np.random.choice(attack_indices[i], int(len(attack_indices[i]) * DIVIDE_RATE)) for i in range(CLASSNUM - 1)])
...
def model_structure():
    ...
    # model, model_name = BoBoNet(model_name='BoBoNet_CICIDS(RS)').model_CICIDS()
    model, model_name = BoBoNet(model_name='Net_CICIDS').model_CICIDS()
...
if __name__ == '__main__':
    # run(r'K:\dataset\CIC-IDS-2017\4_Dataset\unbalanced\train')
    run(r'K:\dataset\CIC-IDS-2017\4_Dataset\train')
